...
const COMMAND_PALETTE_ACTIONS = [
  ...
  { id: 'test', label: 'Rodar testes', hint: 'npm test', run: () => runDevCommand('test') },
  {
    ...
    label: 'Git: status',
    hint: 'git status',
    run: () => runDevCommand('git-status'),
  },
  { id: 'git-diff', label: 'Git: diff', hint: 'git diff', run: () => runDevCommand('git-diff') },
  {
    id: 'fix',
    label: 'Corrigir ultimo erro com Nexus',
    hint: 'Debug Agent',
    run: () => fixLastCommandWithNexus(),
  },
  {
    id: 'patches',
    label: 'Abrir Patch Review',
    hint: 'Patches',
    run: () => openPatchesPanel?.({ viewDiff: true }),
  },
  {
    id: 'explorer',
    label: 'Mostrar Explorer',
    hint: 'Arquivos',
    run: () => activateSideView('explorer'),
  },
  { id: 'search', label: 'Mostrar Busca', hint: 'Arquivos', run: () => activateSideView('search') },
  {
    id: 'git',
    label: 'Mostrar Git',
    hint: 'Controle de versao',
    run: () => activateSideView('git'),
  },
  {
    id: 'settings',
    label: 'Mostrar Configuracoes',
    hint: 'IA',
    run: () => activateSideView('settings'),
  },
