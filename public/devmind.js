...
(function (global) {
  ...
  function authHeaders(extra = {}) {
    const headers = { ...extra };
    const token = global.NexusIDE?.getLocalAuthToken?.();
    if (token) {
      headers['X-Nexus-Request'] = 'true';
  ...
  function renderPendingPatchSticky() {
    const last = [...state.history]
      .reverse()
      .find((m) => m.role === 'assistant' && m.patchIds?.length);
  ...
  function renderTechDetails(data) {
    ...
    if (data.ai?.provider)
      lines.push(`Motor: ${data.ai.provider}${data.ai.model ? ' / ' + data.ai.model : ''}`);
  ...
  function renderPlanBox(planData) {
    return `<div class="dm-confirm-box" style="border-left: 4px solid var(--yellow); background: rgba(255,193,7,0.05); margin-top: 10px;">
      ...
      <div class="dm-confirm-actions" style="margin-top:12px;">
        <button class="dm-btn dm-btn-accent" data-plan-action="confirm" data-plan-goal="${esc(planData.goal)}">Gerar agora</button>
        <button class="dm-btn" style="color:var(--yellow)" data-plan-action="adjust">Ajustar plano</button>
        <button class="dm-btn dm-btn-danger" data-plan-action="cancel">Cancelar</button>
      </div>
    </div>`;
  }
  ...
  function renderMessages() {
    ...
    body.querySelectorAll('[data-plan-action]').forEach((node) => {
      node.addEventListener('click', () => {
        const action = node.getAttribute('data-plan-action');
        if (action === 'confirm') confirmPlan(node.getAttribute('data-plan-goal') || '');
        if (action === 'adjust') adjustPlan();
        if (action === 'cancel') cancelPlan();
      });
    });
  ...
  async function sendText(text) {
    ...
    try {
      ...
      if (!modeRes.ok || !modeData.ok)
        throw new Error(modeData.error || 'Falha ao classificar tarefa');
  ...
  function buildSystemPrompt(decision) {
    return [
      'Voce e o Nexus Codex, um assistente profissional de programacao com IA.',
      'O usuario espera codigo de nivel produto, nao exemplos basicos.',
      'Se o usuario pedir site, app, tela, componente, API ou correcao, gere uma proposta completa, revisavel e segura.',
      'Criterios: design adulto, responsividade, acessibilidade, estados reais, nomes claros, tratamento de erro e zero placeholders pobres.',
      'Nunca aplique mudancas diretamente. O Nexus usa staged files, patches e Patch Review.',
      `Modo atual: ${decision.mode}. Agentes: ${decision.agents.join(', ')}.`,
      'Quando modificar arquivos, gere acoes estruturadas para o backend criar patch. Nao finja que editou arquivo.',
    ].join('\n');
  }
  ...
  function renderShell(containerId) {
    ...
    el.querySelectorAll('[data-suggest]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        if (v === 'Ver patches') {
          dispatch('devmind:action', { id: 'open_patches', type: 'view', value: 'patches' });
          return;
        }
        sendText(v);
      });
  ...
  const DevMind = {
    ...
    showPlan(planPatch) {
      const lastUserMsg = state.history
        .filter((m) => m.role === 'user' && !m.content.includes('++CONFIRM_PLAN++'))
        .pop();
