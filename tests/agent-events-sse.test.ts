...
import { TEST_TOKEN, authHeaders, configureTestSecurity } from './helpers.js';
...
describe('agent run event endpoints', () => {
  ...
  it('streams existing and new run events as text/event-stream', async () => {
    ...
    const response = await request(app)
      .get(`/api/agents/runs/${run.id}/events/stream?nexusToken=${TEST_TOKEN}`)
      .buffer(true)
    ...
    expect(body).toMatch(
      /"type":"(started|planning|reading_project|tool_call|needs_approval|completed|failed)"/,
    );

    const legacy = await request(app)
      .get(`/api/agents/runs/${run.id}/events`)
      .set(authHeaders())
      .expect(200);
