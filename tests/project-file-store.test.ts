...
describe("project-file-store", () => {
  ...
  it("renames a file without overwriting an existing destination", async () => {
    ...
    await expect(readProjectFile(projectRoot, "src/new.ts")).resolves.toMatchObject({ content: "old" });

    await expect(renameProjectPath(projectRoot, "src/new.ts", "src/existing.ts")).rejects.toThrow("Destino ja existe");
  });

  it("deletes files and folders", async () => {
    await writeProjectFile(projectRoot, "docs/remove.md", "temporary");
    await expect(deleteProjectFile(projectRoot, "docs/remove.md")).resolves.toMatchObject({ path: "docs/remove.md" });
