...
function detectFramework(packageJson: { dependencies?: Record<string, string>; devDependencies?: Record<string, string> }) {
  const deps = {
...
export function readProjectSnapshot(projectRootInput = "."): ProjectSnapshot {
  ...
  const branch = runGitCommand(resolved.absoluteRoot, ["branch", "--show-current"]).stdout || "Sem Git";
  const gitStatusLines = runGitCommand(resolved.absoluteRoot, ["status", "--short"]).stdout
    .split("\n")
    .map((line) => line.trim())
    .filter(Boolean);
  const lastCommit =
    runGitCommand(resolved.absoluteRoot, ["log", "-1", "--pretty=%h %s"]).stdout || "Sem commits detectados";
