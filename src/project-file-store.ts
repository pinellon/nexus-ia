...
async function walkTree(baseDir: string, rootDir: string, limit: { count: number }): Promise<ProjectTreeNode[]> {
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    if (BLOCKED_DIRECTORIES.has(normalizedName) || isBlockedName(normalizedName) || isBlockedExtension(entry.name)) {
      continue;
    }
