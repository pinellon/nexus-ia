...
export class AnthropicProvider {
  ...
  async chat(messages: AIMessage[]): Promise<AIProviderResponse> {
    ...
    const response = await client.messages.create({
      ...
      max_tokens: 900,
      system,
      messages: conversation.length ? conversation : [{ role: "user", content: "Analise o projeto." }]
    });
    ...
    return {
      ...
      model: this.model,
      content,
      inputTokens: response.usage.input_tokens ?? estimateTokens(messages.map((message) => message.content).join("\n")),
