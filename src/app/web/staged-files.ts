...
const DATA_DIR = resolveNexusDataPath();
const DB_PATH = resolveNexusDataPath('staged-files.json');
const BACKUPS_DIR = resolveNexusDataPath('backups', 'staged-files');
...
export async function addStagedFile(
  input: Omit<StagedFile, 'id' | 'createdAt' | 'status' | 'versions'> & {
...
export async function applyStagedFile(projectRoot: string, id: string) {
  ...
  if (file?.projectRoot) {
    ...
    if (activeRoot !== stagedRoot) {
      throw new Error(
        'Este patch aponta para fora do projeto ativo e foi bloqueado por seguranca.',
      );
    }
  }
  ...
  if (file.baselineContent === null && exists) {
    throw new Error(
      `O arquivo ${file.path} foi criado desde a proposta. Recalcule o patch antes de aplicar.`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `O arquivo ${file.path} mudou desde a proposta. Salve/revise e recalcule antes de aplicar.`,
    );
  }
