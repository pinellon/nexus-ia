...
export function suggestAgentId(goal: string): string {
  ...
  if (
    ...
    !/(criar|crie|gerar|gere|novo|nova|site|app|landing|componente|página|pagina|tela)/.test(lowered)
  ) {
    return "debug_agent";
  }
  ...
  if (/(testar|testes|vitest|jest|coverage)/.test(lowered) && !/(criar|crie|site|app)/.test(lowered)) {
    return "test_agent";
  }
...
export function isCodeCreationGoal(goal: string): boolean {
  ...
  if (/(apenas docs|somente readme|só documenta|only docs|documenta[çc][aã]o|readme)/.test(normalized) &&
